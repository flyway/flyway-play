...
class PlayInitializer @Inject() (
  ...
  private lazy val flyways: Map[String, Flyway] = {
    for {
      ...
      migrationFilesLocation = s"$flywayPrefixToMigrationScript/${configuration.dbFolder.getOrElse(dbName)}"
  ...
  private def migrationDescriptionToShow(dbName: String, migration: MigrationInfo): String = {
    ...
    (if (locations.nonEmpty) locations.map(location => environment.resourceAsStream(s"$flywayPrefixToMigrationScript/${flywayConfigurations(dbName).dbFolder.getOrElse(dbName)}/$location/${migration.getScript}"))
      .find(resource => resource.nonEmpty).flatten
    else {
      environment.resourceAsStream(s"$flywayPrefixToMigrationScript/${flywayConfigurations(dbName).dbFolder.getOrElse(dbName)}/${migration.getScript}")
    }).map { in =>
