...
class UrlParserSpec extends AnyFunSpec with Matchers {
  ...
  describe("UrlParser") {
    ...
    it("should parse URI that starts with 'mysql:' and has no extra parameters") {
      urlParser.parseUrl("mysql://john:secret@host.example.com/dbname") should be(
        (
          "jdbc:mysql://host.example.com/dbname?useUnicode=yes&characterEncoding=UTF-8&connectionCollation=utf8_general_ci",
          Some("john"),
          Some("secret")
        )
      )
